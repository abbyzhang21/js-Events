...
function moreContent(){
	var oneElem = document.getElementById('more');
	oneElem.innerHTML = bacon;
}
...
function zoom(){
	var biggieElem = document.getElementById('biggie');
	biggieElem.style.fontSize = "150%";
}
...
var menuItems = ["mcferry","icecream","coffee","pie"];
function valueMenu(){
...
Benjamin.addEventListener('click', showMyQuote);
function showMyQuote(){
	var displayQuoteElem = document.getElementById('displayQuote');
	displayQuoteElem.innerHTML = myQuote;
}
...
//create randomly select from arry
var randomQuote = quotes[Math.floor(Math.random()*quotes.length)];
random.addEventListener('click', showRandomQuote);
function showRandomQuote(){
	var displayQuotesElem = document.getElementById('displayQuotes');
	displayQuotesElem.innerHTML = randomQuote;
}
...
*/
showHide.addEventListener('click', showOfFreedom);
function showOfFreedom(){
	var freedomElem = document.getElementById('freedom');
	if(freedomElem.style.display === 'none'){
		freedomElem.style.display = 'block';
	}else{
		freedomElem.style.display = 'none';
	}
}
